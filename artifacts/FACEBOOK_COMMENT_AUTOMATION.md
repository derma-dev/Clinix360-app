...
| 3b | **`parent_id` is set on EVERY Facebook comment — top-level too.** For a top-level comment `parent_id === post_id` (the parent IS the post); only a reply-in-a-thread has `parent_id !== post_id`. Unlike Instagram, which only sets `parent_id` for real threads. The extractor nulls it when it equals `post_id`, or every top-level comment gets skipped as "threaded" (the bug that hid the first real test comment). |
