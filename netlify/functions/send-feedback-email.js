...
// Emails admin when a staff member submits feedback
// ============================================================
...
exports.handler = async (event) => {
  ...
  const { branch_name, entry_date, feedback_text, submitted_by } = body;
  ...
  const senderLabel = submitted_by ? submitted_by : 'Anonymous';
  ...
  const emailRes = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [ADMIN_EMAIL],
      subject: `💬 Staff Feedback — ${branch_name} (${entry_date})`,
      html: `
        <div style="font-family:Arial,sans-serif;max-width:460px;margin:0 auto;padding:32px 24px;color:#1f2937">
          ...
          <div style="background:#f0fdf4;border:1px solid #86efac;border-radius:10px;padding:16px 20px;margin-bottom:20px;text-align:center">
            <div style="font-size:13px;color:#166534;font-weight:600;text-transform:uppercase;margin-bottom:4px">New Staff Feedback</div>
            <div style="font-size:15px;font-weight:600;color:#15803d">${branch_name}</div>
          </div>
          <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:16px 20px;margin-bottom:20px;font-size:15px;line-height:1.6;color:#374151">
          ...
          <table style="width:100%;border-collapse:collapse;font-size:14px">
            ...
            <tr>
              ...
              <td style="padding:10px 0;font-weight:600;text-align:right">${senderLabel}</td>
            </tr>
          </table>
          <p style="margin:20px 0 0;color:#9ca3af;font-size:12px;text-align:center">Check the Admin Panel → Notifications for all feedback.</p>
        </div>
      `,
    }),
  });
